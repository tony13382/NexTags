...
class SmartPlaylist(BaseModel):
    ...
    exclude_tags: List[str] = Field(default_factory=list, description="排除標籤條件")
    filter_language: List[str] = Field(default_factory=list, description="語言過濾條件")
    exclude_language: List[str] = Field(default_factory=list, description="排除語言條件")
    ...
    exclude_tags_display: List[str] = Field(default_factory=list, description="排除標籤顯示名稱")
    filter_language_display: List[str] = Field(default_factory=list, description="語言顯示名稱")
    exclude_language_display: List[str] = Field(default_factory=list, description="排除語言顯示名稱")
...
class SmartPlaylistCreate(BaseModel):
    ...
    filter_tags: List[str] = Field(default_factory=list, description="標籤過濾條件")
    exclude_tags: List[str] = Field(default_factory=list, description="排除標籤條件")
    filter_language: List[str] = Field(default_factory=list, description="語言過濾條件 (例如: chi, eng, kor)")
    exclude_language: List[str] = Field(default_factory=list, description="排除語言條件 (例如: chi, eng, kor)")
    filter_favorites: Optional[bool] = Field(None, description="我的最愛過濾條件")
    sort_method: str = Field("creation_time", description="排序方式: creation_time 或 title (titlesort)")
    is_system_level: bool = Field(False, description="系統等級播放清單：True = 存入 /Music/Playlist，False = 存入 /Music/{base_folder}/Playlist")
...
class SmartPlaylistUpdate(BaseModel):
    ...
    exclude_tags: Optional[List[str]] = Field(None, description="排除標籤條件")
    filter_language: Optional[List[str]] = Field(None, description="語言過濾條件 (例如: chi, eng, kor)")
    exclude_language: Optional[List[str]] = Field(None, description="排除語言條件 (例如: chi, eng, kor)")
    filter_favorites: Optional[bool] = Field(None, description="我的最愛過濾條件")
